# PROMPT=$'%(?.%F{green}✅.%F{red}❌%?%f) %F{yellow}%n%f@%m in %F{blue}%d%f $(__git_ps1 "(%s)") \n%# '
# RPROMPT="$(blueutil --connected) \
# 📶$(networksetup -getairportnetwork en0 | awk '{print substr($0,24)}'):$(airport --getinfo | awk '/agrCtlRSSI/ {print $2}') \
...
#   local bottom_right='%F{yellow}%T%f'
  ...
  local top_left="%(?.%F{green}✅.%F{red}❌%?%f) %F{yellow}%n%f@%m in %F{blue}%d%f ${git_info} "
  local botton_right=""
  local bottom_left='%B%F{%(?.green.red)}%#%f%b '
  local top_right="🕘%t \
  $(blueutil --connected) \
